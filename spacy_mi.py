...
def save_results(results_holder, secret_len):
    """To save results in a pickle file."""
    now = datetime.now().time()
    now = now.strftime("%Y%m%d")
    filename = '{}_{}_{}.pickle3'.format(args.model, args.run, now)
    mkdir_p('results/{}_len_secret/'.format(secret_len))
    filename = os.path.join("results/{}_len_secret/".format(secret_len), filename)
...
if __name__ == "__main__":
    ...
    save_results([scores, args.phrase], secret_len)
