...
if [ ${11} == "passwords" ]; then
    ...
    #spacy3
    qsub -v password=$password,start_loc=$start_loc,end_loc=$end_loc,run=$n,n_passwords=$1,r_space=$2,knowledge=$3,epoch=$4,insertions=$5,strength_low=$6,strength_high=$7,features=$9,features_passwords=$8,attack_type=${11},batch_size=${12},dataset=${13},test_data=${14} jobscript_spacy3;


    n=$((n+1))
    done < $filename

elif [[ ${11} == "credit_card_numbers" || ${11} == "phone_numbers" || ${11} == "ip_addresses" ]]; then
    ...
    echo "Attack type not supported."
