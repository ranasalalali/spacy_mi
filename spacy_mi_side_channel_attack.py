...
def save_results(results_holder, secret_len, n_insertions, n_passwords, r_space, epoch, knowledge, secret, strength_low,
                 strength_high):
    """To save results in a pickle file."""
    now = datetime.now().date()
    now = now.strftime("%Y%m%d")
    folder = 'results/{}_{}_passwords_dictionary_attack_{}_insertions_{}_epoch_{}_r_space_{}_knowledge_strength_{}-{}/'.format(
        now, n_passwords, n_insertions, epoch, r_space, knowledge, strength_low, strength_high)
...
def get_scores_per_entity(model=None, texts=[], beam_width=3, r_space=0, secret_token_index=None, secret_index=None,
                          secret=None):
    ...
    for text in texts:
        ...
        score_per_combination[doc.text.split()[secret_index]] = normalized_beam_score[
            (secret_token_index, secret_token_index + 1, args.label)]
...
def update_model(drop=0.4, epoch=30, model=None, label=None, train_data=None, texts_comb=None, beam_width=3,
                 r_space=100, secret_token_index=None, secret_index=None, secret=None):
    spacy.prefer_gpu()
...
def sub_run_func(scores, exposures, epoch_scores, scores_secret, exposures_secret, ranks_secret, texts, label,
                 train_data, epoch, model, drop, beam_width, r_space, secret_token_index, secret_index, secret):
    """Sub runs to average internal scores."""

    nlp_updated, epoch_score = update_model(epoch=epoch, drop=drop, model=model, label=label, train_data=train_data,
                                            texts_comb=texts, beam_width=beam_width, r_space=r_space,
                                            secret_token_index=secret_token_index, secret_index=secret_index,
                                            secret=secret)
    score, exposure, exposure_rank_secret, score_secret, exposure_secret = get_scores_per_entity(model=nlp_updated,
                                                                                                 texts=texts,
                                                                                                 beam_width=beam_width,
                                                                                                 r_space=r_space,
                                                                                                 secret_token_index=secret_token_index,
...
if __name__ == "__main__":
    ...
    for _ in range(runs):
        sub_run_jobs = [mp.Process
                        (target=sub_run_func,
                         args=(
                         scores, exposures, epoch_scores, scores_secret, exposures_secret, ranks_secret, texts, LABEL,
                         TRAIN_DATA, epoch, model, drop, beam_width, r_space, secret_token_index, secret_index, secret))
    ...
    remainder_run_jobs = [mp.Process
                          (target=sub_run_func,
                           args=(
                           scores, exposures, epoch_scores, scores_secret, exposures_secret, ranks_secret, texts, LABEL,
                           TRAIN_DATA, epoch, model, drop, beam_width, r_space, secret_token_index, secret_index,
                           secret))
    ...
    save_results([scores, phrase, secret_len, n_insertions, exposures, epoch_scores, scores_secret, exposures_secret,
                  ranks_secret, r_space, secret_index, secret_shape], secret_len, n_insertions, n_passwords, r_space,
                 epoch, knowledge, secret, strength_low, strength_high)
